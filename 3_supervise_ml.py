...
𝑘 nearest neighbors regression can be performed with sklearn's KNeighborsRegressor model object, https://scikit-learn.org/stable/modules/generated/sklearn.neighbors.KNeighborsRegressor.html.
"""
...
soil = pd.read_excel(r"soil_nutrient_data_copy.xlsx")
print(soil)
# 0 are nebraska location
# 1 is oklahoma location
...
soil_train, soil_test = train_test_split(soil,
                                            random_state=431,
                                            shuffle=True,
                                            test_size=.2,
                                            stratify=soil['soil_class'])

## Plotting the training data
## yield and pH, but you can change which features you want to map
plt.figure(figsize=(8,6))

plt.scatter(soil_train.loc[soil_train.soil_class==0].Yield,
            soil_train.loc[soil_train.soil_class==0].pH,
            c='blue',
            s=60,
            label="0")

plt.scatter(soil_train.loc[soil_train.soil_class==1].Yield,
            soil_train.loc[soil_train.soil_class==1].pH,
            c='orange',
            s=60,
            marker='v',
            label="1")
...
knn.fit(soil_train[['Yield', 'OM', 'pH', 'N', 'P', 'K', 'Ca', 'Mg', 'S', 'Na', 'Fe', 'Zn', 'Mn', 'Cu']],
        soil_train.soil_class)

## predict on the training set
knn.predict(soil_train[['Yield', 'OM', 'pH', 'N', 'P', 'K', 'Ca', 'Mg', 'S', 'Na', 'Fe', 'Zn', 'Mn', 'Cu']])
...
accuracy(soil_train.soil_class.values,
         knn.predict(soil_train[['Yield', 'OM', 'pH', 'N', 'P', 'K', 'Ca', 'Mg', 'S', 'Na', 'Fe', 'Zn', 'Mn', 'Cu']]))

## Get the training accuracy for our model here
accuracy_score(soil_train.soil_class.values,
                 knn.predict(soil_train[['Yield', 'OM', 'pH', 'N', 'P', 'K', 'Ca', 'Mg', 'S', 'Na', 'Fe', 'Zn', 'Mn', 'Cu']]))
